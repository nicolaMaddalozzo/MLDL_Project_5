...
def main():
    ...
    if args.results_folder is not None:
        ...
        results_folder_path = "results/PPO_target_lr_" + str(args.lr) + "_gae_" + str(args.gae) + "_ntimesteps_" + str(args.n_timesteps)
    ...
    if args.save_every is not None:
        checkpoint_callback = CheckpointCallback(save_freq=args.save_every, save_path=results_folder_path + "/logs/", name_prefix='ppo_model')
    else:
        checkpoint_callback = None
    
    from stable_baselines3 import PPO
    
    model = PPO("MlpPolicy", env, gae_lambda=args.gae, learning_rate=args.lr, gamma=0.99, verbose=1)
    ...
    if not os.path.exists(results_folder_path + "/model"):
        ...
        model.save(results_folder_path + "/model/ppo_model.mdl")
